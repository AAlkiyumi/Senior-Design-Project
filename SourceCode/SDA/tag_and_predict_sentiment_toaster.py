...
def main(starting_df, appliance_type, tag_file, sentiment_topics):
    ...
    # review_df = excelSheetToDf(r"outputs\sentiment_outputs\Slice Toaster Sentiment Breakdown 2024_2_20.xlsx", "Reviews")
    # sentences_df = excelSheetToDf(r"outputs\sentiment_outputs\Slice Toaster Sentiment Breakdown 2024_2_20.xlsx", "Sentences")
    ...
    # tagged_reviews_df, tagged_sentences_df = gpt_tagger.main("inputs/tag_lists/Slice Toaster GPT Tag List.xlsx", review_df, sentences_df, "outputs/sentiment_outputs/" + appliance_type + " GPTagged " + str(datetime.now().year) + "_" + str(datetime.now().month) + "_" + str(datetime.now().day) + ".xlsx", appliance_type)
    ...
    # tagged_reviews_df = excelSheetToDf(r"outputs\sentiment_outputs\Slice Toaster GPTagged 2024_2_27 gpt-4-0125-preview.xlsx", "Reviews")
    # tagged_sentences_df = excelSheetToDf(r"outputs\sentiment_outputs\Slice Toaster GPTagged 2024_2_27 gpt-4-0125-preview.xlsx", "Sentences")
    ...
    tagged_and_predicted_review_df = excelSheetToDf(r"outputs\sentiment_outputs\Slice Toaster Tags and Sentiment Predictions 2024_4_22.xlsx", "Reviews")
    tagged_and_predicted_sentence_df = excelSheetToDf(r"outputs\sentiment_outputs\Slice Toaster Tags and Sentiment Predictions 2024_4_22.xlsx", "Sentences")
...
if __name__ == "__main__":
    ...
    starting_df = pd.read_excel("outputs/Reviews/Combined Slice Toaster Reviews 2024_4_19.xlsx")

    appliance_type = "Slice Toaster"
    tag_file = "inputs/tag_lists/Slice Toaster GPT Tag List.xlsx"
    sentiment_topics = ['Appearance', 'Performance', 'Noise', 'Speed', 'Quality', 'Durability', 'Reliability', 'Efficiency', 'Design', 'Size', 'Ease of Use', 'Safety', 'Temperature Control', 'Power Consumption', 'Versatility', 'Brand', 'Price', 'Cleaning', 'Capacity', 'Portability', 'Customer Service', 'Accessories', 'Flavor', 'Weight', 'Application', 'Energy Efficiency', 'Residue', 'Complexity']
