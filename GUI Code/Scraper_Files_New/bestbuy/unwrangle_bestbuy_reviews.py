...
from Scraper_Files_New.bestbuy import s3_read_and_write
import itertools
...
def main(product_df, API_KEY, DESTINATION_BUCKET, DAYS_AGO):
    BASE_URL = "https://data.unwrangle.com/api/getter/"
    today = datetime.today().date()
    CUTOFF_DATE = today - timedelta(DAYS_AGO)
    df = product_df
    df.drop_duplicates(subset='id', inplace=True)
    REVIEW_DESTINATION_KEY = 'raw-scraping-outputs/bestbuy/reviews/' + str(datetime.now()) + '.csv'

    # Use ThreadPoolExecutor for multithreading
    review_data_list = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        results = list(executor.map(fetch_review_data, df['url_x'], itertools.repeat(API_KEY), itertools.repeat(BASE_URL), itertools.repeat(CUTOFF_DATE)))
    ...
    reviews_df.to_csv('bestbuy_product_reviews.csv')
