...
def get_products_df(search_term, api_key):
    ...
    while (page_number <= max_pages) and (retries < 2):
        print("Page number: ", page_number, "Max pages: ", max_pages)
    ...
    response_df.to_csv('bestbuy_product_search_response_df.csv')

    return response_df

# Function to fetch product data
def fetch_product_data(product_id, api_key, destination_bucket):
...
def main(SEARCH_TERM, API_KEY, DESTINATION_BUCKET):
    PRODUCT_DESTINATION_KEY = 'raw-scraping-outputs/bestbuy/products/' + str(datetime.now()) + '.csv'
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        results = list(executor.map(fetch_product_data, products_df['id'], itertools.repeat(API_KEY), itertools.repeat(DESTINATION_BUCKET)))
