...
REVIEW_DESTINATION_KEY = 'raw-scraping-outputs/lowes/reviews/' + str(datetime.now()) + '.csv'
...
def get_review(product_url, API_KEY, CUTOFF_DATE):
    ...
    while True:
        ...
        # call the api
        url = 'https://data.unwrangle.com/api/getter/?platform=lowes_reviews&url=' + str(product_url) + '&page=' + str(page_number) + '&api_key=' + str(API_KEY)
...
def main(product_df, DESTINATION_BUCKET, API_KEY, DAYS_AGO):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        results = list(executor.map(fetch_review_data, df['url'], itertools.repeat(API_KEY), itertools.repeat(CUTOFF_DATE)))
    ...
    reviews_df.to_csv('lowes_product_reviews.csv')
