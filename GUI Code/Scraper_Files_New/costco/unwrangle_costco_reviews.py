...
def get_review(product_url, API_KEY, BASE_URL, CUTOFF_DATE):
...
def fetch_review_data(product_url, API_KEY, BASE_URL, CUTOFF_DATE):
    """Fetch reviews for a product and cross join with product details"""
    try:
        return get_review(product_url, API_KEY, BASE_URL, CUTOFF_DATE)  # get_review() is assumed to return a DataFrame
    except Exception as e:
        print(f"Error fetching reviews for {product_url}: {e}")
        return pd.DataFrame()  # Return empty DataFrame if error occurs


def main(product_df, API_KEY, REVIEW_DESTINATION_BUCKET, DAYS_AGO):
    REVIEW_DESTINATION_KEY = 'raw-scraping-outputs/costco/reviews/' + str(datetime.now()) + '.csv'
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        results = list(executor.map(fetch_review_data, df['url'], itertools.repeat(API_KEY), itertools.repeat(BASE_URL), itertools.repeat(CUTOFF_DATE)))
